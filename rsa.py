...
def encrypt_file(filename, outfile, keyprefix=keypair_prefix):
    pkey = '{}_public.key'.format(filename)
    skey = '{}_private.key'.format(filename)
    
    print('Encrypting file {} using Key from files {}, {}'.format(filename, pkey, skey))
    (e,n),(d,n) = load_keys(keyfile)
    out = open(outfile, 'w')
    with open(filename) as f:
        for l in f.readlines():
            out.write('{}\n'.format(encrypt_num(int(l.strip()), (e,n))))
        f.close()
    out.close()
...
if __name__ == '__main__':
    ...
    if args[1].lower() == 'genkey':
        ...
        num_entries = 100
        if arglen < 3:
            print('Usage: {} {} {}'.format(args[1], '<Out file>', '[<Num Entries>]'))
            exit()
